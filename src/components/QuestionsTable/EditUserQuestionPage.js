...
const EditUserQuestionPage = () => {
    ...
    useEffect(() => {
        questionService.getQuestion(id, user.token).then((data) => {
            setQuestion(data);
        });
    }, []);
    ...
    const updateHandler = (event) => {
        ...
        questionService.approveQuestion(id, question, trueAnswer, wrongAnswerOne, wrongAnswerTwo, wrongAnswerThree, user.token)
            .then((data) => {
                if (data.error) {
                    ...
                    navigate("/admin/unapproved-questions-table")
                }
